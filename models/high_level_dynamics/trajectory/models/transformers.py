...
class GPT(nn.Module):
    ...
    def _set_pretrained_gpt(self, model):
        self.gpt_with_olddata = model
    ...
    def forward(self, idx, targets=None, mask=None, old_logits=None):
        """
            idx : [ B x T ]
            values : [ B x 1 x 1 ]
        """
...
class ConditionalGPT(GPT):
    ...
    def __init__(self, config):
        ...
        ## to the sequence
        config.block_size += config.goal_dim #config.observation_dim
        super().__init__(config)
        #self.goal_emb = nn.Embedding(config.vocab_size * config.observation_dim, config.n_embd)
        self.goal_emb = nn.Embedding(config.vocab_size * config.goal_dim, config.n_embd)

    def get_block_size(self):
        return self.block_size - self.goal_dim #self.observation_dim
    ...
    def forward(self, idx, goal, targets=None, mask=None):
        ...
        gx = self.blocks(gx)
        x = gx[:, self.goal_dim:] #gx[:, self.observation_dim:]
        #### /goal
