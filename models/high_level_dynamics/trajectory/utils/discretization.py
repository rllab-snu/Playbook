...
class QuantileDiscretizer:
	...
	def _test(self):
		...
		# print(np.sum(np.where(indices==indices_2, False, True))) # randint = np.random.randint(0, self.N, indices.shape)
		# print(indices_2.shape) # randint_2 = self.discretize(self.reconstruct(randint))
		#assert (indices == indices_2).all() # assert (randint == randint_2).all()
		# print('✓')
...
class MixedDiscretizer:

	def __init__(self, data, N, thresholds=None):
		self.data = data
		self.N = N
